...
echo "==> purge the stable loaders so dealers pick up $SHA"
for f in dist/loader/*.js; do
  name=$(basename "$f" .js)
  ok=0
  for try in 1 2 3; do
    curl -s --max-time 60 "$PURGE@main/dist/loader/$name.js" -o /dev/null
    live=$(curl -sL --max-time 60 "$CDN@main/dist/loader/$name.js" | grep -oE "@[0-9a-f]{7,40}/" | head -1 | tr -d '@/')
    if [ "$live" = "$SHA" ]; then echo "    loader/$name  -> $SHA"; ok=1; break; fi
  done
  [ "$ok" = 1 ] || echo "    loader/$name  still serving ${live:-unknown} — purge lag; dealers pick it up within ~12h or re-run the purge"
done
